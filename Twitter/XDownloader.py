...
class TwitterDownloaderApp:
    ...
    def download_photo(self, url, download_dir):
        try:
            ...
            self.update_status(f"Downloaded photo: {filename}")
        except Exception as e:
            self.update_status(f"Error downloading photo: {str(e)}")
    ...
    def download_video(self, url, download_dir):
        try:
            ...
            filename = f"video_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{os.urandom(4).hex()}.mp4"
            ...
            self.update_status(f"Downloaded video: {filename}")
        except Exception as e:
